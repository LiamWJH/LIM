...
LIM is a dynamically & strongly typed programming language designed for prototyping softwares fast.

It's similar to Python and its design has been influenced by Python, Rust and Typescript.
...
Strings can be concatenated by the `+` operator and be used as an argument for the `length()` function.
...
An array is not homogeneous in LIM. An array can contain items with different types.
